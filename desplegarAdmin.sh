...
if [ $parametro = "preproduccion" ] || [ $parametro = "Preproduccion" ] || [ $parametro = "PREPRODUCCION" ]; then
sudo docker container run -d --name veganfood_frontend -v /opt/veganfood/html/frontend:/usr/local/apache2/htdocs -p 10330:80 httpd:2.4.41
elif [ $parametro = "produccion" ] || [ $parametro = "produccion" ] || [ $parametro = "PRODUCCION" ]; then
    docker container run -d --name veganfood_frontend -v /opt/veganfood/html/frontend:/usr/local/apache2/htdocs --expose 80 -e VIRTUAL_HOST=www.admin.veganfood.pve2.fpmislata.com -e LETSENCRYPT_HOST=www.admin.veganfood.pve2.fpmislata.com --net "nginx-net" httpd:2.4.41
fi
