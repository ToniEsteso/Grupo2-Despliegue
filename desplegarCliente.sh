...
sudo docker container run -d --name veganfood_frontend -v /opt/veganfood/html/frontend/web:/usr/local/apache2/htdocs -p 10320:80 httpd:2.4.41
    ...
    docker container run -d --name veganfood_frontend -v /opt/veganfood/html/frontend/web:/usr/local/apache2/htdocs --expose 80 -e VIRTUAL_HOST=www.veganfood.pve2.fpmislata.com -e LETSENCRYPT_HOST=www.veganfood.pve2.fpmislata.com --net "nginx-net" httpd:2.4.41
fi
